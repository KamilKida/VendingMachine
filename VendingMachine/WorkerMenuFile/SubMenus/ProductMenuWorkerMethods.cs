...
namespace VendingMachine.WorkerMenu.SubMenus
{
    internal class ProductMenuWorkerMethods : ProductDisplay 
    {
        ...
        /// </summary>
        /// <param name="place">Id/place in vending machine of product.</param>
        ...
        /// <param name="dateName">Depending on choic allows to change one value of product. Options are: 'name' for name, 'price' for price, 'amount' for amount.</param>
            ...
            foreach (var product in products)
            {
                if (product.place == place)
                {
                    ...
                    if (dateName == "name")
                    {
                        ...
                        {
                            Console.Clear();
                            worker.AddAction($"Changed product name: {product.name} ,to: {newName}.  ");
                            ...
                            else 
                            {
                                try
                                {
                                    if(decimal.Parse(newPrice) >= 0 ) {
                                        ...
                                        {
                                            Console.Clear();
                                            worker.AddAction($"Changed product : '{product.name}' price: {product.price} $ ,to: {newPrice} $.  ");
                    ...
                    else if (dateName == "amount")
                    {
                            ...
                            while (true)
                            {
                                ...
                                string newAount = Console.ReadLine();
                                if (newAount.ToLower() == "x") { Console.Clear(); Console.WriteLine($"Amount of a product: '{product.name}' was not changed. \n"); ToContinue(); break; }
                            try
                                {
                                    if (int.Parse(newAount) >= 0)
                                    {
                                        if (int.Parse(newAount) <= 10)
                                        {
                                            Console.Clear();
                                            worker.AddAction($"Changed product: '{product.name}' amount: {product.amount} ,to: {newAount}.  ");
                                            product.ChangeDate("amount", newAount);
                                ...
                                {
                                    Console.Clear();
                                    Console.WriteLine($"You have input something diffrent then number: {newAount}. \n"); ToContinue(); continue;
                                }
                            }
        ...
        /// </summary>
        /// <returns>Bool type: 'true' if there are at least one, 'false' if there is none.</returns>
        ...
        public void RefillProduct(string workerChoice, int productId, WorkerObj worker)
        {
            ...
            {
                worker.AddAction("Refilled all products in vending machine to full capacity. ");
                foreach (var product in products)
                {
                    if (product.name != "EMPTY")
                    {
            ...
            else if (workerChoice == "1" && productId != 0)
            {
                foreach (var product in products)
                {
                    ...
                    {
                        worker.AddAction($"Refilled product: {product.name} to full slot capacity. ");
                        product.ChangeDate("amount", 10.ToString());
                    }
                }
            }
            else if (workerChoice == "2" && productId != 0)
            {
                ...
                while (amountToken == true)
                {
                        ...
                        foreach (var product in products)
                        {
                            ...
                            if (product.place == productId)
                            {
                                if ((int.Parse(newAmount) + product.amount) <= 10)
                                {
                                    ...
                                    Console.Clear();
                                    worker.AddAction($"Refilled product: {product.name} to capacity: {(int.Parse(newAmount) + product.amount)}. ");
                    ...
                    catch (Exception e)
                    {
                        ...
                        Console.Clear(); Console.WriteLine($"You have input something that is not on the list: {newAmount}. \n"); ToContinue();
                    }
                }
        ...
        /// <summary>
        /// Adds new product in place of product with name: 'EMPTY'. If there is already 10 products in machine with name different than: 'EMPTY' it will no allow to add new product. 
        /// Product takes place of first instance of product with name: 'EMPTY'.
        /// </summary>
        /// <param name="worker">'WorkerObj' object used for adding actions to worker using: 'AddAction()'</param>
            ...
            if (occupiedSlots < 10)
            {
                ...
                while (priceToken == true)
                {
                    ...
                    try
                    {
                        ...
                        {
                            Console.Clear();
                            Console.WriteLine("Price cant be lower then 0 $. \n");
                            ToContinue();
                            continue;
                        }
                    }
                    ...
                    foreach(var product in products)
                {
                    ...
                    if (product.name == "EMPTY")
                    {
                        ...
                        worker.AddAction($"Added new product: {product.name} to slot: {product.place}. ");
                ...
                if (!emptyToken)
                {
                    ...
                    worker.AddAction($"Added new product: {newProduct.name} to slot: {newProduct.place}. ");
        ...
        /// <summary>
        /// Allows worker to delete product from list by changing his name to: 'EMPTY' , price and amount to zero.
        /// </summary>
        /// <param name="worker">'WorkerObj' object used for adding actions to worker using: 'AddAction()'</param>
                    ...
                    case "y":
                        foreach(var product in products)
                        {
                            if(product.place == productId)
                            {
                                ...
                                worker.AddAction($"Deleted product: {product.name} from slot: {product.place}. ");
