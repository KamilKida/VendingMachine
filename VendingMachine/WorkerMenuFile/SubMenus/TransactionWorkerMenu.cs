...
namespace VendingMachine.WorkerMenu.SubMenus
{
    internal class TransactionWorkerMenu : DefaultOptions
    {
            ...
            if (workerInput.ToLower() != "x")
            {
                while (transactionsToken == true)
                {
                    ...
                    switch (workerInput.ToLower())
                    {
                        case "t":
                            Console.Clear();
                            ISQLcomunication.GetDate(TransactionsInfo, "select top 3 [Date Of Transaction], [Lenght Of transaction], Profits from Transactions order by [Date Of Transaction] desc");
                            worker.AddAction("Looked at last three transactions. ");
                            ToContinue();
                            break;
                        case "a":
                            Console.Clear();
                            ISQLcomunication.GetDate(TransactionsInfo, "select [Date Of Transaction], [Lenght Of transaction], Profits from Transactions order by [Date Of Transaction] desc");
                            worker.AddAction("Looked at all transactions. ");
                            ToContinue();
                            break;
                        case "c":
                            Console.Clear();
        ...
        private void DeleteTransacionhistory(WorkerObj worker)
        {
            ...
            {
                Console.Clear();
                worker.AddAction("Deleted entire history of transactions. ");
