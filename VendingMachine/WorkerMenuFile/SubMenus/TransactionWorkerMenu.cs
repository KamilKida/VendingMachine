            ...
            if (workerInput.ToLower() != "x")
            {
                while (transactionsToken == true)
                {
                    ...
                    switch (workerInput.ToLower())
                    {
                        case "t":
                            Console.Clear();
                            ISQLcomunication.GetData(TransactionsInfo, "select top 3 [Date Of Transaction], [Lenght Of transaction], Profits from Transactions order by [Transaction ID] desc");
