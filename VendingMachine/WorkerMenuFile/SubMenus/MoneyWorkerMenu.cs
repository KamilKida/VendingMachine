...
namespace VendingMachine.WorkerMenu.SubMenus
{
    internal class MoneyWorkerMenu : DefaultOptions
    {
        ...
        private void WithdraAllMoney(WorkerObj worker)
        {
            ...
            {
                Console.WriteLine("You can not withdraw money from vending machine because there is 0,00 $ inside. ");
                Console.WriteLine(" ");
            ...
            else
            {
                while (true)
                {
                    ...
                    if (choice.ToLower() == "y") {
                        ...
                        ISQLcomunication.SendData("UPDATE [Money In Machine] SET [Current Money In Machine] = 0 WHERE [Money ID]= 1");
