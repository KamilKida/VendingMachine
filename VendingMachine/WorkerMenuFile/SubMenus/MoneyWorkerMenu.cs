...
namespace VendingMachine.WorkerMenu.SubMenus
{
    internal class MoneyWorkerMenu : DefaultOptions
    {
        ...
        public void MoneyMenuInterface(string name, WorkerObj worker)
        {
            Console.Clear();
            string workerInput = MoneyMenuLook(name);
            ...
            if(workerInput.ToLower() != "x") {
                while (moneyToken == true)
                {
                    if(firstRun != true) { 
                    Console.Clear();
                    workerInput = MoneyMenuLook(name);
                    }
                    firstRun = false;
                    ...
                    switch (workerInput.ToLower())
                    {
                        case "s":
                            Console.Clear();
                            ShowMoney();
                            worker.AddAction("Looked at money in vending machine. ");
                            ToContinue();
                            break;
                        case "w":
                            Console.Clear();
        ...
        public string MoneyMenuLook(string name)
        {
            string optins = $"s: Show money in vending machine. \n" +
                            $"w: Withdraw all money in vending machine.";

            DefaultMenu("MONEY OPTIONS", name, optins, false,true);
        ...
        /// <summary>
        /// Shows money in vending machine.
        /// </summary>
        ...
        private void WithdraAllMoney(WorkerObj worker)
        {
            ...
            else
            {
                while (true)
                {
                    ...
                    if (choice.ToLower() == "y") {
                        Console.Clear();
                        worker.AddAction($"Withdraw all money: {moneyInMachine[0]} $ from vending machine: ");
                        ISQLcomunication.SendDate("UPDATE [Money In Machine] SET [Current Money In Machine] = 0 WHERE ID = 1");
