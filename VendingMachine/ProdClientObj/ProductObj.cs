...
namespace VendingMachine.ProdClientObj
{
    internal class ProductObj
    {
        ...
        /// <summary>
        /// Shows all values of: 'ProductObj' object.
        /// </summary>
        ...
        /// </summary>
        /// <param name="information">Decides what kind of value will be changed or actions taken. Options are: 'name' for name, 'price' for price, 'amount' for amount, 'add' for adding new produc, 'deleted' for deleting product. </param>
        /// <param name="newInformation">New value you want to be set for product.</param>
        ...
        public void ChangeDate(string information, string newInformation)
        {
            if(information == "name")
            {
                ...
                ISQLcomunication.SendData($"UPDATE Products SET [Product Name] = '{newInformation}' where [Product ID] = {this.place}");
            }
            else if(information == "price")
            {
                ...
                ISQLcomunication.SendData($"UPDATE Products SET [Product Price In $] = '{newInformationFormat}' where [Product ID] = {this.place}");
            }
            else if (information == "amount")
            {
                this.amount = int.Parse(newInformation);
                ISQLcomunication.SendData($"UPDATE Products SET [Amount In Machine] = '{int.Parse(newInformation)}' where [Product ID] = {this.place}");
            }
            else if(information == "deleted")
            {
                ...
                this.amount = 0;
                this.price = 0;
                ISQLcomunication.SendData($"UPDATE Products SET [Product Name] = '{this.name}' , [Amount In Machine] = {this.amount}, [Product Price In $] = {this.price} where [Product ID] = {this.place}");
            }
        }
